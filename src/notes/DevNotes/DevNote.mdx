...
SpecTag: DevNotes
desc: >-
  TODO:Add Dynamic Loading / Pagination (Mansory-like) with dev node script or
  vite pluginWon't it be
---
...
{/* <TryDynamicRoutingComponent /> */}
 ...
 - 今天用 Typst 替换了 Katex 进行数学公式的渲染，使用的是
   [@myriaddreamin/rehype-typst](https://www.npmjs.com/package/@myriaddreamin/rehype-typst)
   . MyriadDreamin 实在是 Typst 领域推广大神。
   最近也在 Obsidian 中使用 Typst，比 tex 的一堆反斜杠还是舒服太多。

   -- 2025/11/24
 - 今天把 Typst 的内容渲染也加进去了，可以顺利渲染一个完整的 Typst 文档。随后可以把之前的笔记扔过来。
   依然是 Fork myriaddreamin 的插件，略微调整了渲染的模板。
   后面打算实现对 Typst import 的支持，这样就可以把之前的笔记拿来直接渲染了。
