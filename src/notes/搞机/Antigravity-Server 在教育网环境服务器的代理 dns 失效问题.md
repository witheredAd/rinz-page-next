...
> 写在后面：
> 
> 后来发现这个配置还是存在问题：Clash里的dns配置了fakeip，导致如果dns从clash里获取，就必须走clash代理才能连到网络。依然没有完全满足我想要的仅我可用的透明代理。
> 
> 一个最常见的问题就是apt，sudo apt 更新的时候由于没有自动读取环境变量，导致apt从clash的dns获取到fakeip，但是无法链接。这样，即使配置了镜像源，apt还是必须经过clash。
